...
function ensureState(state) {
  if (!state || typeof state !== "object") {
    return { users: [], invites: [], connections: [], gameSessions: [] };
  }
...
function upsertUser(state, payload) {
  const nextState = ensureState(state);
  const { displayName, handle, email } = validateUserInput(nextState, payload);
  const existingUser = nextState.users.find((user) => user.handle === handle) || null;
...
function buildDashboard(state, userId) {
  ...
  const receivedInvites = nextState.invites
    ...
    .map((invite) => ({
      ...invite,
      senderHandle: findUserById(nextState, invite.senderUserId)?.handle || "unknown",
    }));
